...
class Darknet_DMS(Darknet):
    """
    Darknet model for DMS dataset.
    This class is a specialized version of Darknet for the DMS dataset.
    It uses different in_features and has a different output structure.
    """
    depth2blocks = {21: [1, 4, 4, 4], 53: [2, 8, 8, 4]}
    def __init__(
        self, 
        depth, 
        in_channels=3, 
        ...
        super().__init__(depth=depth, in_channels=in_channels,
                         stem_out_channels=stem_out_channels, out_features=out_features)
        assert out_features, "please provide output features of Darknet"
        self.out_features = out_features
        self.stem = nn.Sequential(
        ...
        num_blocks = Darknet_DMS.depth2blocks[depth]
        # create darknet with `stem_out_channels` and `num_blocks` layers.
        # to make model structure more clear, we don't use `for` statement in python.
        self.dark2 = nn.Sequential(
    ...
    def make_group_layer(self, in_channels: int, num_blocks: int, stride: int = 1):
        ...
        return [
            ...
            *[(ResLayer(in_channels * 2, act="relu")) for _ in range(num_blocks)],
        ]
    ...
    def make_group_layer2(self, in_channels: int, num_blocks: int, stride: int = 1):
        ...
        return [
            ...
            *[(ResLayer(in_channels, act="relu")) for _ in range(num_blocks)],
        ]
